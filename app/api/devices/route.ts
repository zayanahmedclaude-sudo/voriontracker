...
export async function PATCH(req: NextRequest) {
  ...
  if (!id || body?.action !== 'revoke') return err('A device id and revoke action are required', 400);
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id)) return err('Device id is invalid', 400);
  await ensureMonitoringSchema();
